        ...
        match anchor_kind(attr) {
            ...
            Some(AnchorKind::Verifies) => {
                if !is_test {
                    ...
                    // The closed oracle set holds in the checker too: a
                    // malformed or unknown opt-out the macro would reject
                    // must not satisfy the gate through an uncompiled
                    // file.
                    let suppression = {
                        shallguard::enforces_here!("REQ-TRACE-017");
                        match oracle_argument(attr) {
                            OracleArgument::Absent => None,
                            OracleArgument::Malformed(reason) => {
                                anchors.invalid.push(InvalidAnchor {
                                    file: file.to_path_buf(),
                                    line: attr_line(attr),
                                    message: format!("{reason} on `{ident}`; suppression ignored"),
                                });
                                None
                            }
                            OracleArgument::Class(class)
                                if !oracle::ORACLE_CLASSES.contains(&class.as_str()) =>
                            {
                                let accepted = oracle::ORACLE_CLASSES
                                    .iter()
                                ...
                                anchors.invalid.push(InvalidAnchor {
                                    ...
                                    line: attr_line(attr),
                                    message: format!(
                                        "unknown oracle class {class:?} on `{ident}` is \
                                         not a suppression; accepted values: {accepted}"
                                    ),
                                });
                                None
                            }
                            OracleArgument::Class(class) => Some(class),
                        }
                    };
...
/// Result of parsing `oracle = <value>` arguments off a
/// `#[shallguard::verifies]` attribute.
enum OracleArgument {
    Absent,
    Class(String),
...
/// Extracts the `oracle` opt-out from a `#[shallguard::verifies]`
/// attribute's argument tokens. String literals are parsed through
/// `syn`, so raw strings decode to their value; duplicates and
/// non-string values are malformed, matching what the macro accepts.
#[shallguard::enforces("REQ-TRACE-014", "REQ-TRACE-017")]
fn oracle_argument(attr: &syn::Attribute) -> OracleArgument {
    let syn::Meta::List(list) = &attr.meta else {
        return OracleArgument::Absent;
    };
    ...
    while i < trees.len() {
        ...
        if found.is_some() {
            return OracleArgument::Malformed("duplicate oracle argument");
        }
        match trees.get(i + 2) {
            Some(proc_macro2::TokenTree::Literal(value)) => match syn::Lit::new(value.clone()) {
                syn::Lit::Str(lit) => found = Some(lit.value()),
                _ => {
                    return OracleArgument::Malformed("oracle value must be a string literal");
                }
            },
            _ => return OracleArgument::Malformed("oracle value must be a string literal"),
        }
        i += 3;
    }
    match found {
        Some(class) => OracleArgument::Class(class),
        None => OracleArgument::Absent,
    }
}
...
#[shallguard::enforces("REQ-TRACE-019")]
fn id_search_text(tokens: proc_macro2::TokenStream) -> String {
    ...
    while i < trees.len() {
        ...
        if is_oracle_eq {
            i += 2;
            while i < trees.len()
                && !matches!(&trees[i], proc_macro2::TokenTree::Punct(comma) if comma.as_char() == ',')
            {
                i += 1;
            }
            if i < trees.len() {
