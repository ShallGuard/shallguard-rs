...
embedded in the top-level `README.md`; `migration-bootstrap.gif` and
`migration-ratchet.gif` are embedded in `docs/MIGRATION.md`
(regenerated by the separate `regenerate-migration.sh`).
They are recorded with [VHS](https://github.com/charmbracelet/vhs)
